...
def build_knn(points: List[Point], k: int = 15) -> Dict[Point, List[Point]]:
    ...
    P = np.asarray(points, dtype=float)
    tree = cKDTree(P)
    dists, idxs = tree.query(P, k=min(k+1, len(points)))
    knn = {}
...
def best_bridge_between_sets(A: List[Point], B: List[Point], limit: int = 25) -> Tuple[Tuple[Point,Point], float]:
    if not A or not B:
        return (None, None), float('inf')
    def centroid(S):
        sx = sum(p[0] for p in S)/len(S); sy = sum(p[1] for p in S); sy/=len(S)
        return (sx, sy)
...
def read_bks_file(bks_path: str) -> Dict[str, Dict[str, float]]:
    """
    Lee /src/BKS/BKS_TSP.txt y devuelve {instancia_normalizada: {'bks': float, 'note': str}}
    Soporta líneas con anotaciones, p.ej. 'dsj1000 : 18659688 (EUC_2D)'.
    Si hay duplicados (EUC_2D vs CEIL_2D), prioriza EUC_2D.
    """
    bks = {}
    if not os.path.exists(bks_path):
        print(f"[WARN] No se encontró BKS en {bks_path}")
        return bks
    prefer_euc = {}
    with open(bks_path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            name = _norm_name(left)
            right = right.strip()
            note = ""
            val_str = right
            if "(" in right and ")" in right:
                val_str, note_raw = right.split("(", 1)
                note = note_raw.rsplit(")", 1)[0].strip()
            ...
            try:
                ...
                continue
            if name not in bks:
                bks[name] = {"bks": value, "note": note}
                prefer_euc[name] = ("EUC_2D" in note.upper())
            else:
                is_euc = ("EUC_2D" in (note or "").upper())
                if is_euc and not prefer_euc.get(name, False):
                    bks[name] = {"bks": value, "note": note}
                    prefer_euc[name] = True
    return bks
def compute_gap_rows(results: List[Dict], bks_map: Dict[str, Dict[str, float]]) -> pd.DataFrame:
    """
    results: lista de dicts que devuelve run_single_instance o run_all_instances
    bks_map: dict de read_bks_file
    """
    rows = []
    for r in results:
        inst_raw = r.get("instance", "")
        name = _norm_name(inst_raw)
        bks_val = None
        bks_note = ""
        if name in bks_map:
            bks_val = bks_map[name]["bks"]
            bks_note = bks_map[name].get("note", "")
        length = r["total_length"] if r.get("success", False) else None
        gap = None
        if bks_val is not None and length is not None and bks_val > 0:
            gap = (float(length) - float(bks_val)) / float(bks_val)
        rows.append({
            "instancia": inst_raw,
            "puntos": r.get("points", None),
            "longitud_total": length,
            "BKS": bks_val,
            "gap_%": gap,           
            "tiempo_s": r.get("execution_time", None),
            "exitoso": r.get("success", False),
            "nota_BKS": bks_note,
            "error": r.get("error", "")
        })
    df = pd.DataFrame(rows)
    cols = ["instancia","puntos","longitud_total","BKS","gap_%","tiempo_s","exitoso","nota_BKS","error"]
    return df[cols]
def save_excel_with_gap(results: List[Dict], bks_path: str, out_dir: str = RESULTS_DIR) -> str:
    os.makedirs(out_dir, exist_ok=True)
    bks_map = read_bks_file(bks_path)
    df = compute_gap_rows(results, bks_map)
    df_sorted = df.sort_values(by=["exitoso","gap_%","tiempo_s"], ascending=[False, True, True], na_position="last")
    xlsx_path = os.path.join(out_dir, "resumen_consolidado.xlsx")
    with pd.ExcelWriter(xlsx_path, engine="xlsxwriter") as writer:
        df_sorted.to_excel(writer, index=False, sheet_name="Resultados")
        wb  = writer.book
        ws  = writer.sheets["Resultados"]
        fmt_pct = wb.add_format({"num_format": "0.00%"})
        fmt_int = wb.add_format({"num_format": "0"})
        fmt_flt = wb.add_format({"num_format": "0.000"})
        headers = list(df_sorted.columns)
        for col_idx, col_name in enumerate(headers):
            if col_name == "gap_%":
                ws.set_column(col_idx, col_idx, 10, fmt_pct)
            elif col_name in ("tiempo_s",):
                ws.set_column(col_idx, col_idx, 12, fmt_flt)
            elif col_name in ("puntos","longitud_total","BKS"):
                ws.set_column(col_idx, col_idx, 14, fmt_int)
            else:
                ws.set_column(col_idx, col_idx, 18)
        ws.write(0, len(headers), "generado_en")
        ws.write(1, len(headers), datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    print(f"✓ Excel con GAP guardado en: {xlsx_path}")
    return xlsx_path
def setup_command_line_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--bks_path", type=str, default="src/BKS/BKS_TSP.txt",
                        help="Ruta al archivo BKS para calcular GAP (default: src/BKS/BKS_TSP.txt)")
    parser.set_defaults(tsp="a280.tsp")
    return parser
def validate_and_prepare_parameters(args) -> Tuple[Optional[str], bool, float, int, bool, bool, str]:
    if args.all:
        run_all = True
        tsp_file = None
    else:
    ...
    bks_path = args.bks_path
    return tsp_file, run_all, eps_frontier, max_iter_local, save_plots, show_plots, bks_path
...
def main():
    ...
    tsp_file, run_all, eps_frontier, max_iter_local, save_plots, show_plots, bks_path = validate_and_prepare_parameters(args)
    ...
    if run_all:
        ...
        if results:
            ...
            save_excel_with_gap(results, bks_path, RESULTS_DIR)
        ...
        save_excel_with_gap(single_result, bks_path, RESULTS_DIR)
        # plot
        if save_plots:
